  ...
  const loadDNSResults = async (assetId = selectedAsset, options = {}) => {
    try {
      ...
      if (data && data.result_data) {
        const parsedDNS = JSON.parse(data.result_data);
        const formattedDNS = [];

        // Đổ a_records vào mảng
        if (parsedDNS.a_records) {
          parsedDNS.a_records.forEach(ip => formattedDNS.push({ type: 'A', value: ip }));
        }
        // Đổ mx_records vào mảng
        if (parsedDNS.mx_records) {
          parsedDNS.mx_records.forEach(mx => formattedDNS.push({ type: 'MX', value: mx.Host || mx }));
        }

        setDnsData(formattedDNS);
        setDnsPagination({
          total: formattedDNS.length, 
          page: 1,
          page_size: pageSize,
          total_pages: 1,
        });
